...
public class MasterKategori extends javax.swing.JPanel {
    ...
    try {
        String sql = "SELECT * FROM kategori LIMIT ? OFFSET ?";
        try (PreparedStatement st = conn.prepareStatement(sql)) {
    ...
    String sql = "SELECT SUBSTR(ID_Kategori, -3) AS Nomor " +
                 "FROM kategori " +
    ...
    String sql = "SELECT Nama_Kategori FROM kategori WHERE ID_kategori != ? AND Nama_Kategori = ? COLLATE NOCASE";
    try (PreparedStatement st = conn.prepareStatement(sql)){
